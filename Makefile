...
EXP_DIRS:=$(sort $(dir $(wildcard $(addsuffix /Makefile,$(addprefix experiments/,$(TARGETS))))))
$(EXP_DIRS): lib
	@printf "  BUILD   $@\n";
...
clean: $(EXP_DIRS:=.clean) styleclean
	$(Q)$(MAKE) -C libopencm3 clean

stylecheck: $(EXP_DIRS:=.stylecheck)
styleclean: $(EXP_DIRS:=.styleclean)
...
.PHONY: build lib examples $(EXP_DIRS) install clean stylecheck styleclean \
